...
class _ResetPINState extends State<ResetPIN> {
  ...
  _submit() async {
    ...
    if (_konfirmSandiController.text != pinBaru) {
      await h.failAlert("Konfirmasi PIN Baru Tidak Cocok", "Harap periksa kembali PIN dan konfirmasi PIN Baru Anda!",);
  ...
  Widget build(BuildContext context) {
    return _isProcessing ? UiLoader(label: "Mengubah PIN ...",) : Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
      ...
      UiInput("PIN Baru", isRequired: true, icon: LineIcons.lock, info: "6-digit", type: UiInputType.PIN, controller: _sandiController, focusNode: _sandiFocusNode),
      UiInput("Konfirmasi PIN Baru", isRequired: true, icon: LineIcons.lock, type: UiInputType.PIN, controller: _konfirmSandiController, focusNode: _konfirmSandiFocusNode),
      SizedBox(height: 25,),
      Center(child: UiButton("Simpan", width: 150, height: 46, color: Colors.green, icon: LineIcons.check, iconSize: 30, iconRight: true, onPressed: _submit,)),
    ],);
  }
}