  ...
  Widget withBadge(int? num, {bool showNum = true, Alignment alignment = Alignment.topRight, Color bgColor = Colors.red, double fontSize = 14.0, Color fontColor = Colors.white, Color borderColor = Colors.red, double borderWidth = 2.0, show = true}) {
    return Stack(alignment: alignment, children: <Widget>[
      ...
      show && (num ?? 0) > 0 ? Card(
        ...
        color: bgColor,
        child: showNum ? Text("$num", style: TextStyle(fontSize: fontSize, color: fontColor),).withPadding(5) : const SizedBox(width: 12, height: 12,)
      ).withOffset(8, -8) : const SizedBox(),
    ],);
  }
...
class ShimmerIt extends StatefulWidget {
  const ShimmerIt({Key? key, required this.child, this.minOpacity = 0.4, this.maxOpacity = 0.8, this.shimmer = true}) : super(key: key);
  final Widget child;
...
class PulseIt extends StatefulWidget {
  const PulseIt({Key? key, required this.child, this.pulse = true, this.scaleBegin = 1.0, this.scaleEnd = 1.2, this.duration = 500}) : super(key: key);
  final Widget child;
  final bool pulse;
